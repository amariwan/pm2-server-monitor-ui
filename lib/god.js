...
/* Using the si library to get the CPU information and then logging it to the console. */
var CPU;
si.cpu().then((data) => (CPU = data)).catch((error) => console.error(error));
// io.use(
...
io.on('connection', (socket) => {
	...
	if (sessionUser) {
		...
		socket.on('stopApp', (appName) => {
			console.log('stopApp', appName, sessionUser.UserRole);
			if (sessionUser.UserRole) {
				pm2.stop(appName, (err, x) => {
					if (err) return socket.emit('serverInfo', 'error', err);
					else socket.emit('serverInfo', 'success', x[0].name + ' ' + x[0].status);
				});
			} else socket.emit('serverInfo', 'error', x[0].name + ' ' + x[0].status);
		});
