				...
				_setContent: function() {
					...
					if (t.icon && t.icon != null) {
						var n = e(t.icon).is('img')
							? e(t.icon)
							: e(t.icon).find('img').size() != 0 ? e(t.icon).find('img') : e(t.icon);
				...
				_poisition: function() {
					...
					if (i > 1) {
						...
						switch (pS[1]) {
							case 'top':
								o.top =
									parseFloat(e('body').find('.notify.' + n).eq(i - 2).css('top')) +
									e('body').find('.notify.' + n).eq(i - 2).outerHeight() +
									t.spacing;
								break;
							case 'bottom':
								o.bottom =
									parseFloat(e('body').find('.notify.' + n).eq(i - 2).css('bottom')) +
									e('body').find('.notify.' + n).eq(i - 2).outerHeight() +
									t.spacing;
								break;
						}
